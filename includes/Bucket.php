...
class Bucket {
    ...
    public function createOrModifyTable(string $bucketName, object $jsonSchema) {
        $newSchema = array_merge(array(), self::$requiredColumns);
    ...
    private function getAlterTableStatement($dbTableName, $newSchema, $oldSchema) {
        ...
        return "ALTER TABLE $dbTableName " . join($alterTableFragments, ", ") . ";";
    }
    ...
    private function getCreateTableStatement($dbTableName, $newSchema) {
        ...
        return "CREATE TABLE $dbTableName (" . join($createTableFragments, ', ') . ");";
    }
    ...
    public function getCategoriesCondition($condition, &$categoryMap) {
        if (self::isOrAnd($condition)) {
            ...
            $children = join($children, " {$condition['op']} ");
            return "($children)";
        } else if (self::isNot($condition)) {
    ...
    public function getWhereCondition($condition, $fieldNamesToTables, $schemas, $dbw) {
        if (self::isOrAnd($condition)) {
            ...
            return "(NOT $child)";
        } else if (is_array($condition) && is_array($condition[0])) {
            // .where{{"a", ">", 0}, {"b", "=", "5"}})
            return self::getWhereCondition(["op" => "AND", "operands" => $condition], $fieldNamesToTables, $schemas, $dbw);
        } else if (is_array($condition) && !empty($condition) && !isset($condition[0])) {
            // .where({a = 1, b = 2})
            $operands = [];
        ...
        } else if (is_array($condition) && isset($condition[0]) && isset($condition[1])) {
    ...
    public function runSelect($data) {
        ...
        foreach ($data['joins'] as $join) {
            ...
            $jsonObject = "JSON_ARRAYAGG(JSON_OBJECT(" . join($jsonObjectFragments, ", ") . "))";
